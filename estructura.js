...
function setupUI() {
  ...
  tipoEl.textContent = currentUser.tipo_usuario === 'archivo' ? 'Archivo Central' : 'Personal Activo';
  ...
  // Mostrar secciones del sidebar según rol
  document.querySelectorAll('.tab-solo-archivo').forEach(t => {
    t.style.display = currentUser.tipo_usuario === 'archivo' ? (t.classList.contains('sidebar-seccion') ? 'block' : 'flex') : 'none';
  });
  document.querySelectorAll('.tab-solo-activo').forEach(t => {
    t.style.display = currentUser.tipo_usuario === 'activo' ? (t.classList.contains('sidebar-seccion') ? 'block' : 'flex') : 'none';
  });
...
function cambiarTab(tabId) {
  ...
  const titulos = { dashboard:'Inicio', inventario:'Inventario', 'solicitudes-archivo':'Solicitudes de Salida', 'nuevos-archivo':'Nuevos Expedientes', devoluciones:'Control de Devoluciones', 'mis-solicitudes':'Mis Solicitudes', 'mis-prestamos':'Mis Préstamos', 'nuevo-expediente':'Registrar Expediente', remisiones:'Remisiones', historial:'Historial de Movimientos', reportes:'Reportes' };
  const topbarTitulo = document.getElementById('topbar-titulo');
  if (topbarTitulo) topbarTitulo.textContent = titulos[tabId] || tabId;
...
async function cargarDashboard() {
  try {
    ...
    document.getElementById('actividad-reciente').innerHTML =
      '<div class="empty-state"><div class="icon"><i class="ph ph-warning-circle"></i></div><h4>Error de conexión</h4><p>Verifique la configuración de Supabase en estructura.js</p></div>';
  }
}
...
function renderizarStatsporTipo(porTipo) {
  ...
  const labels = {
    solicitudes_dotacion: { icon:'<i class="ph ph-file-text"></i>', label:'Solicitudes de Dotación' },
    autorizadas:          { icon:'<i class="ph ph-seal-check"></i>', label:'Autorizadas' },
    adjudicaciones:       { icon:'<i class="ph ph-house-line"></i>', label:'Adjudicaciones' },
    usufructos:           { icon:'<i class="ph ph-tree-evergreen"></i>', label:'Usufructos' },
    determinativas:       { icon:'<i class="ph ph-gavel"></i>', label:'Determinativas' },
  };
...
async function cargarActividadReciente() {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = '<div class="empty-state"><div class="icon"><i class=\"ph ph-list-checks\"></i></div><h4>Sin actividad reciente</h4></div>'; return;
  }
  const iconos = { solicitud_salida:'', salida:'', solicitud_devolucion:'', devolucion:'', ingreso_nuevo:'', ingreso_nuevo_aceptado:'', remision:'', devolucion_remision:'↩️' };
...
function renderizarInventario(data, tipo) {
  ...
  if (!data || data.length === 0) {
    tbody.innerHTML = `<tr><td colspan="9"><div class="empty-state"><div class="icon"><i class=\"ph ph-folder-open\"></i></div><h4>Sin expedientes</h4></div></td></tr>`;
    return;
  }
  ...
  tbody.innerHTML = data.map(exp => {
    ...
    let acciones = `<button class="btn btn-sm btn-info" onclick="verDetalleExpediente('${tipo}',${exp.id})">Ver</button>`;
  ...
  if (!historial || historial.length === 0) {
    ...
    html += historial.map(m => {
      ...
      if (m.expediente_actualizado)
        updateHtml = `<div class="alerta-update mt-1"><span class="icon"><i class="ph ph-warning"></i></span><div class="texto">Actualización: <strong>${m.cuerpos_anterior}→${m.cuerpos_nuevo} cuerpos</strong>, <strong>${m.fojas_anterior}→${m.fojas_nuevo} fojas</strong></div></div>`;
      return `
        <div class="historial-item">
          <div class="historial-header">
          ...
          <div class="historial-body">
            ${(()=>{
              const esNuevo = m.tipo_movimiento==='ingreso_nuevo'||m.tipo_movimiento==='ingreso_nuevo_aceptado';
              const primero = esNuevo ? m.nombre_solicitante        : m.nombre_personal_archivo;
              const segundo = esNuevo ? m.nombre_personal_archivo   : m.nombre_solicitante;
              return (primero?`<strong>${primero}</strong>`:'')+( segundo?` → <strong>${segundo}</strong>`:'');
            })()}
    ...
    html += rems.map(r => `
      <div class="historial-item" style="border-left:3px solid var(--teal);">
        ...
        <div class="fechas-bloque mb-1">
          ...
          ${r.devuelto && r.fecha_devolucion_rem ? `<div class="fecha-row"><span class="fecha-label">Devuelto el:</span> <strong>${fmtFecha(r.fecha_devolucion_rem)}</strong></div>` : ''}
        ...
        <div class="historial-body">
          ...
          ${r.devuelto ? `<br>Recibido por: <strong>${r.nombre_receptor||'—'}</strong>` : ''}
          ${r.motivo_salida ? `<br>${r.motivo_salida}` : ''}
          ${r.obs_devolucion ? `<br>Obs. devolución: ${r.obs_devolucion}` : ''}
        </div>
        ${r.devuelto ? '<div class="mt-1"><span class="badge badge-devuelto">Devuelto al Archivo</span></div>' : '<div class="mt-1"><span class="badge badge-remitido">En dirección destino</span></div>'}
      </div>`).join('');
  }
...
async function cargarMisSolicitudes() {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = '<div class="empty-state"><div class="icon"><i class=\"ph ph-tray\"></i></div><h4>No tienes solicitudes pendientes</h4><p>Todas tus solicitudes han sido procesadas.</p></div>';
    actualizarBadgePendientes();
    return;
  }
...
async function cargarSolicitudesArchivo() {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = '<div class="empty-state"><div class="icon"><i class=\"ph ph-tray\"></i></div><h4>Sin solicitudes pendientes</h4></div>'; return;
  }
  const items = await Promise.all(data.map(async m => {
    const { data: exp } = await db.from(m.tipo_expediente).select('*').eq('id', m.expediente_id).single();
    if (!exp) return '';
...
async function cargarNuevosParaArchivar() {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = '<div class="empty-state"><div class="icon"><i class=\"ph ph-tray\"></i></div><h4>Sin nuevos expedientes pendientes</h4></div>'; return;
  }
  const items = await Promise.all(data.map(async m => {
    const { data: exp } = await db.from(m.tipo_expediente).select('*').eq('id', m.expediente_id).single();
    if (!exp) return '';
    ...
    return `
      <div class="historial-item" style="border-left:3px solid var(--dorado);">
        ...
        <div class="flex gap-1">
          <button class="btn btn-success btn-sm" onclick="aceptarNuevoExpediente(${m.id},'${m.tipo_expediente}',${m.expediente_id})">Verificar e Ingresar</button>
...
async function aceptarNuevoExpediente(movId, tipo, expId) {
  ...
  if (!exp) return;
  const codigoDisplay = tipo === 'determinativas' ? exp.nro_resolucion : exp.codigo;
  abrirModal('modal-accion', 'Ingresar Nuevo Expediente a Archivo', `
...
async function cargarDevoluciones() {
  ...
  if (!solicDevol?.length && !prestamos?.length) {
    el.innerHTML = '<div class="empty-state"><div class="icon"><i class=\"ph ph-envelope-simple\"></i></div><h4>Sin devoluciones pendientes</h4></div>'; return;
  }
  let html = '';
...
async function renderItemDevolucion(m, esSolicitud) {
  ...
  if (esSolicitud && m.expediente_actualizado)
    updateAlert = `<div class="alerta-update"><span class="icon"><i class="ph ph-warning"></i></span><div class="texto"><strong>El Funcionario actualizó el expediente:</strong><br>Cuerpos: <strong>${m.cuerpos_anterior} → ${m.cuerpos_nuevo}</strong> | Fojas: <strong>${m.fojas_anterior} → ${m.fojas_nuevo}</strong></div></div>`;
  let fechasHtml = '';
...
async function aceptarDevolucion(movId, tipo, expId, actualizado, cuerposNuevo, fojasNuevo) {
  const { data: exp } = await db.from(tipo).select('*').eq('id', expId).single();
  if (!exp) return;
  const alertaUpdate = actualizado ? `<div class="alerta-update mb-2"><span class="icon"><i class="ph ph-warning"></i></span><div class="texto"><strong>Actualización:</strong> Cuerpos: <strong>${exp.cuerpos} → ${cuerposNuevo}</strong> · Fojas: <strong>${exp.numero_fojas} → ${fojasNuevo}</strong></div></div>` : '';
...
async function cargarMisPrestamos() {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = '<div class="empty-state"><div class="icon"><i class=\"ph ph-list-checks\"></i></div><h4>No tienes expedientes en préstamo</h4></div>'; return;
  }
  ...
  const items = await Promise.all(data.map(async m => {
    ...
    if (m.tipo_movimiento === 'solicitud_devolucion') {
      ...
      botonesHtml = `
        ...
        <button class="btn btn-secondary btn-sm" onclick="abrirActualizacion(${m.id},'${m.tipo_expediente}',${m.expediente_id},${exp.cuerpos},${exp.numero_fojas})">Actualizar</button>
...
function abrirActualizacion(movId, tipo, expId, cuerposAct, fojasAct) {
  abrirModal('modal-accion', 'Actualizar Datos del Expediente', `
    <p class="mb-2">Registre los cambios antes de devolver:</p>
    <div class="alerta-update mb-2"><span class="icon"><i class="ph ph-chart-bar"></i></span><div class="texto">Actuales: <strong>${cuerposAct} cuerpos</strong> y <strong>${fojasAct} fojas</strong></div></div>
    <div class="form-grid">
...
async function cargarRemisiones() {
  ...
  if (!data || data.length === 0) { tbody.innerHTML = `<tr><td colspan="10"><div class="empty-state"><div class="icon"><i class=\"ph ph-tray\"></i></div><h4>Sin remisiones registradas</h4></div></td></tr>`; return; }
  ...
  tbody.innerHTML = data.map(r => {
    ...
    const accionHtml = r.devuelto
      ? `<button class="btn btn-sm btn-info" onclick="verDetalleRemision(${r.id})">Ver</button>`
      : `<button class="btn btn-sm btn-info" onclick="verDetalleRemision(${r.id})">Ver</button>
         <button class="btn btn-sm btn-success" onclick="abrirModalRecibirRemision(${r.id})">Recibir</button>`;
    return `
      <tr>
...
async function verDetalleRemision(id) {
  ...
  const html = `
    ...
    ${r.devuelto ? `
      <hr class="divider">
      <h4 style="font-family:Rajdhani,sans-serif;color:var(--verde-inra);margin-bottom:10px;">Devolución Registrada</h4>
      <div class="detalle-grid">
...
async function abrirModalRecibirRemision(remisionId) {
  ...
  abrirModal('modal-accion', 'Registrar Recepción de Expediente Remitido', `
...
async function cargarHistorial(filtros = {}) {
  ...
  if (!data || data.length === 0) {
    el.innerHTML = `<tr><td colspan="13"><div class="empty-state"><div class="icon"><i class=\"ph ph-magnifying-glass\"></i></div><h4>Sin resultados</h4></div></td></tr>`; return;
  }
  ...
  el.innerHTML = (await Promise.all(data.map(async m => {
    ...
    let updateCell = '—';
    if (m.expediente_actualizado)
      updateCell = `<span class="badge badge-reservado">${m.cuerpos_anterior}→${m.cuerpos_nuevo} C / ${m.fojas_anterior}→${m.fojas_nuevo} F</span>`;
    return `<tr>
...
function abrirVentanaImpresion(titulo, desde, hasta, filas, thead) {
  ...
  const fechaGeneracion = ahora.toLocaleDateString('es-BO', { weekday:'long', day:'numeric', month:'long', year:'numeric' });
  const horaGeneracion  = ahora.toLocaleTimeString('es-BO', { hour:'2-digit', minute:'2-digit' });
  const nroDoc = 'DGATF-' + String(ahora.getFullYear()).slice(-2) + '-' + String(ahora.getMonth()+1).padStart(2,'0') + String(ahora.getDate()).padStart(2,'0') + '-' + String(ahora.getHours()).padStart(2,'0') + String(ahora.getMinutes()).padStart(2,'0');

  const periodoHtml = (desde || hasta)
    ? `<td><span class="meta-label">Período</span><span class="meta-val">${desde || '—'} al ${hasta || 'la fecha'}</span></td>`
    : '';

  // Capturar el logo automáticamente desde el DOM del sistema
  const logoImgEl = document.getElementById('header-logo-img');
  const logoSrc   = (logoImgEl && logoImgEl.style.display !== 'none' && logoImgEl.src && !logoImgEl.src.endsWith('undefined'))
    ? logoImgEl.src
    : null;
  const logoHtml  = logoSrc
    ? `<img src="${logoSrc}" alt="INRA" style="width:52px;height:52px;object-fit:contain;display:block;">`
    : `<div style="width:52px;height:52px;background:#064406;display:flex;align-items:center;justify-content:center;color:#D4B030;font-size:9px;font-weight:800;letter-spacing:1.5px;text-align:center;line-height:1.3;">INRA<br>BOL</div>`;
...
<head>
<meta charset="UTF-8">
<title>${titulo} — DGATF · INRA</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Source+Sans+3:wght@300;400;600;700&family=Source+Serif+4:opsz,wght@8..60,400;600&display=swap');
  @page { size: A4 landscape; margin: 14mm 16mm 16mm; }
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .meta-tabla td {
    padding: 5px 10px;
    border-right: 1px solid #d4e8d4;
    vertical-align: top;
  }

  .meta-tabla td:last-child { border-right: none; }
  ...
  table.datos thead th:last-child { border-right: none; }

  table.datos tbody tr:nth-child(odd)  { background: #fff; }
  table.datos tbody tr:nth-child(even) { background: #F7FAF7; }
  ...
  table.datos tbody td:last-child { border-right: none; }
  table.datos tbody td strong { color: #064406; font-weight: 700; }

  table.datos tbody tr:last-child td { border-bottom: none; }

  table.datos tfoot tr { background: #E8F2E8; }
  table.datos tfoot td {
    padding: 5px 7px;
    font-weight: 700;
    border-top: 2px solid #064406;
    font-size: 8.5px;
    color: #064406;
  }
  ...
  .btn-imprimir:hover { background: #043204; }
...
<div class="membrete">
  <div class="membrete-logo">${logoHtml}</div>
  <div class="membrete-institucional">
    <div class="membrete-inst-sup">Estado Plurinacional de Bolivia</div>
    <div class="membrete-inst-nombre">Instituto Nacional de Reforma Agraria</div>
    <div class="membrete-inst-dep">Dirección General de Administración de Tierras Fiscales · Archivo Central</div>
  </div>
  <div class="membrete-nro">
    <div class="nro-label">Referencia</div>
    <div class="nro-val">${nroDoc}</div>
  </div>
</div>

<!-- TÍTULO -->
<div class="rep-titulo-bloque">
  <div>
    <div class="rep-titulo-principal">${titulo}</div>
    <div class="rep-titulo-sub">Sistema de Administración de Archivos — DGATF</div>
  </div>
</div>
...
<table class="meta-tabla">
  <tr>
    <td><span class="meta-label">Fecha de generación</span><span class="meta-val">${fechaGeneracion}, ${horaGeneracion}</span></td>
    ${periodoHtml}
    <td><span class="meta-label">Generado por</span><span class="meta-val">${currentUser.nombre_completo}</span></td>
    <td><span class="meta-label">Cargo</span><span class="meta-val">${currentUser.cargo || 'Personal DGATF'}</span></td>
  </tr>
</table>
...
<div class="pie-sistema">
  INRA Bolivia · Sistema de Administración de Archivos · Ref. ${nroDoc} · ${fechaGeneracion}
</div>
<div class="nota-pie">
  Documento generado automáticamente. Validez sujeta a verificación con registros físicos del Archivo Central.
</div>
...
function abrirModal(modalId, titulo, contenido, footerExtra = '') {
  // Soporte para títulos con íconos HTML
  const tituloEl = document.getElementById(modalId+'-titulo');
  if (tituloEl) {
    // Si el título viene como texto simple, usarlo directamente
    tituloEl.textContent = titulo;
  }
  document.getElementById(modalId+'-body').innerHTML = contenido;
  const footer = document.getElementById(modalId+'-footer');
  if (footer) footer.innerHTML = `${footerExtra}<button class="btn btn-secondary" onclick="cerrarModal('${modalId}')"><i class="ph ph-x"></i> Cancelar</button>`;
...
function mostrarNotif(msg, tipo = 'success') {
  ...
  const iconos    = { success:'<i class="ph ph-check-circle"></i>', error:'<i class="ph ph-x-circle"></i>', warning:'<i class="ph ph-warning"></i>', info:'<i class="ph ph-info"></i>' };
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function actualizarFecha() {
    ...
    if (!el) return;
    const ahora = new Date();
    el.textContent = ahora.toLocaleDateString('es-BO', { weekday:'long', day:'numeric', month:'long', year:'numeric' });
  }
  actualizarFecha();
